...
public class NewsletterService {
    ...
    public Mono<NewsletterResponse> createNewsletter(
        ...
        return newsletterRepository.save(newsletter)
                ...
                .doOnSuccess(response -> log.info("Newsletter {} créée avec succès", response.getId()));
    }
    ...
    public Mono<NewsletterResponse> submitNewsletter(UUID newsletterId, UUID redacteurId) {
        return newsletterRepository.findById(newsletterId)
                ...
                .flatMap(this::mapToResponse)
                .doOnSuccess(response -> log.info(" Newsletter {} soumise pour validation", newsletterId));
    }
    ...
    public Mono<NewsletterResponse> validateNewsletter(UUID newsletterId) {
        return updateNewsletterStatut(newsletterId, StatutNewsletter.VALIDEE)
                .doOnSuccess(response -> log.info("Newsletter {} validée par l'admin", newsletterId));
    }
    ...
    public Mono<NewsletterResponse> rejectNewsletter(UUID newsletterId) {
        return updateNewsletterStatut(newsletterId, StatutNewsletter.REJETEE)
                .doOnSuccess(response -> log.info(" Newsletter {} rejetée par l'admin", newsletterId));
    }
                ...
                .flatMap(newsletter -> {
                    ...
                    getCategoriesForNewsletter(newsletterId)
                            ...
                            .flatMap(categories -> {
                                ...
                                Mono<Void> publishMono = kafkaProducerService
                                        .map(service -> service.publishNewsletterToCategories(event, categories))
                                        .orElseGet(() -> {
                                            log.info(
                                                    "Kafka désactivé: pas de publication d'événement pour la newsletter {}",
                                                    newsletterId);
                                            return Mono.empty();
                                        });

                                return publishMono.thenReturn(publishedNewsletter);
                            }));
                })
                .flatMap(this::mapToResponse)
                .doOnSuccess(response -> log.info(" Newsletter {} publiée et envoyée dans Kafka !", newsletterId));
    }
