...
public class KafkaDynamicListenerManager {
    ...
    private final ConcurrentKafkaListenerContainerFactory<String, NewsletterPublishedEvent> kafkaListenerContainerFactory;
    private final KafkaConsumerService kafkaConsumerService;

    // Cache des containers actifs : groupId -> Container
    private final Map<String, ConcurrentMessageListenerContainer<String, NewsletterPublishedEvent>> activeContainers = new ConcurrentHashMap<>();
     ...
     * @param topics  Liste des topics à écouter (ex: ["newsletter-tech",
     *                "newsletter-sports"])
     */
    public synchronized void createOrGetListener(String groupId, String[] topics) {
        ...
        // Créer le container
        ConcurrentMessageListenerContainer<String, NewsletterPublishedEvent> container = kafkaListenerContainerFactory
        ...
        // Définir le message listeners avec ConsumerSeekAware pour ignorer les anciens
        // messages
        container.setupMessageListener(
                new SeekingMessageListener(kafkaConsumerService, java.util.Arrays.asList(topics)));
    ...
    public synchronized void stopListener(String groupId) {
        ConcurrentMessageListenerContainer<String, NewsletterPublishedEvent> container = activeContainers
                .remove(groupId);
     ...
     */
    public Map<String, ConcurrentMessageListenerContainer<String, NewsletterPublishedEvent>> getActiveContainers() {
