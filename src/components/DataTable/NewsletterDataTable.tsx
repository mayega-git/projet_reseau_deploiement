...
const NewsletterDataTable: React.FC<NewsletterDataTableProps> = ({
  ...
  const renderActions = (row: NewsletterResponse) => (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end">
        ...
        {variant === 'redacteur' && row.id && (
          <>
            ...
            {(row.statut === 'BROUILLON' || row.statut === 'REJETEE') && (
              <DropdownMenuItem
        ...
        {variant === 'admin' && row.id && (
          <>
            {row.statut === 'SOUMISE' && (
              <>
                <DropdownMenuItem
                  onClick={() => handleValidate(row.id as string)}
                  className="gap-2"
                >
              ...
              return (
                <div key={row.id ?? index} className="p-4 space-y-3">
                  ...
                  <p className="paragraph-small-normal text-black-300 break-words">
                    Categories: {truncateText(categoriesLabel || '-')}
                  </p>
                  {variant === 'admin' && (
                    <p className="paragraph-small-normal text-black-300 break-words">
                      Redacteur: {row.redacteurNom || '-'}
                    </p>
                  )}
            ...
            <tbody>
              {rows.map((row, index) => {
                ...
                return (
                  <tr
                    key={row.id ?? index}
                    className={index === rows.length - 1 ? 'rounded-b-[20px]' : ''}
                  >
